 ...
 * 私聊
 */
public class PolyvFragmentPrivateChat extends PolyvBaseChatFragment {
    ...
    private void addQuestionTips() {
        //添加自定义的问候语事件
        chatTypeItems.add(new PolyvChatListAdapter.ChatTypeItem(new PolyvQuestionTipsEvent(), PolyvChatListAdapter.ChatTypeItem.TYPE_RECEIVE));
    }
    ...
    private void sendQuestionMessage() {
        ...
        if (sendMessage.trim().length() == 0) {
            ...
            if (sendValue > 0) {
                ...
                chatTypeItems.add(new PolyvChatListAdapter.ChatTypeItem(questionMessage, PolyvChatListAdapter.ChatTypeItem.TYPE_SEND));
        ...
        disposables.add(PolyvChatEventBus.get().toObservable(EventMessage.class).subscribe(new Consumer<EventMessage>() {
            ...
            public void accept(EventMessage eventMessage) throws Exception {
                ...
                if (eventObject != null && eventType != -1) {
                    chatTypeItems.add(new PolyvChatListAdapter.ChatTypeItem(eventObject, eventType));
                    chatListAdapter.notifyItemInserted(chatListAdapter.getItemCount() - 1);
