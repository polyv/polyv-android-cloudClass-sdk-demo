...
import com.easefun.polyv.cloudclassdemo.watch.chat.widget.PolyvGiftLayout;
import com.easefun.polyv.cloudclassdemo.watch.chat.widget.PolyvRewardLayout;
...
public class PolyvChatGroupFragment extends PolyvChatBaseFragment {
    ...
    private ImageView flower, like, more, selectPhotoButton, openCameraButton, emojioBtn, rewardButton, giftButton;
    private FrameLayout onlyHostSwitch;
    ...
    private LinearLayout q12Layout, flowerLikeLayout;//扣12
    ...
    public static PolyvChatGroupFragment newInstance(String channelId) {
        Bundle args = new Bundle();

        PolyvChatGroupFragment fragment = new PolyvChatGroupFragment();
    ...
    @Override
    protected void updateBottomLayout(boolean showKeyboard) {
        onlyHostSwitch.setVisibility(showKeyboard ? View.GONE : View.VISIBLE);
        emojioBtn.setVisibility(showKeyboard ? View.VISIBLE : View.GONE);
//        emoji.setVisibility(showKeyboard ? View.GONE : View.VISIBLE);

        q12Layout.setVisibility(showKeyboard ? View.VISIBLE : View.GONE);
        flowerLikeLayout.setVisibility(showKeyboard ? View.GONE : View.VISIBLE);
        liv_like.setVisibility(showKeyboard ? View.GONE : View.VISIBLE);
    }

    private void initView() {
        q12Layout = findViewById(R.id.q12_layout);
        flowerLikeLayout = findViewById(R.id.flower_like_layout);
        ...
        q2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sendCustomQMessage(Q2_MESSAGE);
                hideSoftInputAndEmoList();
            }
        });
        emojioBtn = findViewById(R.id.iv_emojileft);
        addPopupBottom(emojioBtn);
        emojioBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                togglePopupLayout(emojioBtn, emoListLayout, getChatEditContainer());
                emoji.setSelected(emojioBtn.isSelected());
            }
        });

        //打赏布局
        PolyvRewardLayout rewardLayout = new PolyvRewardLayout(getContext());
        rewardLayout.setOnPopupWindowDismissListener(new PopupWindow.OnDismissListener() {
            @Override
            public void onDismiss() {
                acceptKeyboardCloseEvent(getCurrentShowPopupLayout(), getChatEditContainer());
            }
        });
        rewardLayout.setOnClickSendRewardButtonListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sendCustomRewardMessage(rewardLayout.getSendCustomRewardBean());
                rewardLayout.hide();
                hideSoftInputAndEmoList();
            }
        });
        ...
        rewardButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                rewardLayout.show(view);
            }
        });

        //礼物布局
        PolyvGiftLayout giftLayout = new PolyvGiftLayout(getContext());
        giftLayout.setOnClickSendGiftButtonListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sendCustomGiftMessage(giftLayout.getSendCustomGiftBean());
                giftLayout.hide();
                hideSoftInputAndEmoList();

                //礼物个数的缩放动效
                zoomGiftCount(chatMessageList.getAdapter().getItemCount() - 1);
            }
        });
        ...
        giftButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                giftLayout.show(view);
            }
        });

        registerGiftLayoutMessage(giftLayout);
        //下拉控件
        ...
        //只看讲师
        onlyHostSwitch = findViewById(R.id.fl_only_see_teacher);
...
//                toast.makeText(getContext(), onlyHostSwitch.isSelected() ? "只看讲师信息" : "查看所有人信息", PolyvToast.LENGTH_LONG).show(true);
        ...
        //点赞
        likeNum = findViewById(R.id.like_icon_num);
        liv_like = findViewById(R.id.liv_like);
        like = findViewById(R.id.like);
        liv_like.setOnButtonClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int sendValue = chatManager.sendLikes(getSessionId());
                if (sendValue < 0) {
...
//        });
    ...
    private void registerGiftLayoutMessage(PolyvGiftLayout giftLayout) {
        giftDispose = PolyvRxBus.get().toObservable(PolyvGiftMessageBean.class).subscribe(new Consumer<PolyvGiftMessageBean>() {
            @Override
            public void accept(PolyvGiftMessageBean polyvGiftMessageBean) throws Exception {
                giftLayout.show(view);
                getChatEditContainer().setVisibility(View.VISIBLE);
            }
        });
    }
    // </editor-fold>
            ...
            if (jsonObject != null) {
                ...
                if (!TextUtils.isEmpty(msgType)) {
                    if (PolyvHistoryConstant.MSGTYPE_CUSTOMMESSAGE.equals(msgType)) {
                        try {
                            PolyvChatListAdapter.ChatTypeItem chatTypeItem = acceptCustomMessage(jsonObject.optString("EVENT"), jsonObject.toString(), PolyvChatManager.SE_CUSTOMMESSAGE, true);
                            if (chatTypeItem != null) {
                                tempChatItems.add(0, chatTypeItem);
                                PolyvCustomEvent customEvent = (PolyvCustomEvent) chatTypeItem.object;
                                if (isOnlyHostType(customEvent.getUser().getUserType(), customEvent.getUser().getUserId())) {
        ...
        if (isHistory) {
            ...
            if (isOnlyWatchTeacher() && listArr[1].size() > 0) {
                ...
                for (int i = 0; i < listArr[0].size(); i++) {
                    int pos = srcMaxPosition + 1 + i;
                    PolyvChatListAdapter.ChatTypeItem chatTypeItem = listArr[0].get(i);
                    if (chatTypeItem.type == PolyvChatListAdapter.ChatTypeItem.TYPE_TIPS) {
                        zoomGiftCount(pos);
                    }
                }
            }
     ...
     * 礼物个数缩放动效
     *
     * @param pos 在RecyclerView中的哪个位置的礼物
     */
    private void zoomGiftCount(int pos) {
        chatMessageList.post(() -> {
            RecyclerView.ViewHolder holder = chatMessageList.findViewHolderForAdapterPosition(pos);
            if (holder instanceof PolyvTipsMessageHolder) {
                PolyvTipsMessageHolder h = (PolyvTipsMessageHolder) holder;
                IPolyvCustomMessageBaseItemView itemView = h.getItemView();
    ...
    private void addSendItemToList(PolyvChatListAdapter.ChatTypeItem chatTypeItem) {
        chatTypeItems.add(chatTypeItem);
        teacherItems.add(chatTypeItem);
        chatListAdapter.notifyItemInserted(chatListAdapter.getItemCount() - 1);
        chatMessageList.scrollToPosition(chatListAdapter.getItemCount() - 1);
    ...
    private void sendCustomRewardMessage(PolyvCustomRewardBean customRewardBean) {
        PolyvCustomEvent<PolyvCustomRewardBean> customEvent = new PolyvCustomEvent<>(PolyvChatMessageEvent.MessageEvent.RewardMessage, chatManager.nickName + "：发送了打赏信息", customRewardBean);
        customEvent.setUser(generateCustomEventUserBean());
        chatManager.sendCustomMsg(customEvent);

        PolyvChatListAdapter.ChatTypeItem chatTypeItem = new PolyvChatListAdapter.ChatTypeItem(customEvent, PolyvChatListAdapter.ChatTypeItem.TYPE_TIPS, PolyvChatManager.SE_CUSTOMMESSAGE);
        addSendItemToList(chatTypeItem);
    }

    private void sendCustomGiftMessage(PolyvCustomGiftBean customGiftBean) {
        PolyvCustomEvent<PolyvCustomGiftBean> customEvent = new PolyvCustomEvent<>(PolyvChatMessageEvent.MessageEvent.GiftMessage, chatManager.nickName + "：发送了送礼信息", customGiftBean);
        customEvent.setUser(generateCustomEventUserBean());
        chatManager.sendCustomMsg(customEvent);
    ...
    private void sendCustomQMessage(int message) {
        PolyvCustomQBean customQBean = new PolyvCustomQBean();
        customQBean.setContentType(message);

        PolyvCustomEvent<PolyvCustomQBean> customEvent = new PolyvCustomEvent<>(PolyvChatMessageEvent.MessageEvent.QMessage, chatManager.nickName + "：发送了扣" + message + "信息", customQBean);
        customEvent.setUser(generateCustomEventUserBean());
        chatManager.sendCustomMsg(customEvent);

        PolyvChatListAdapter.ChatTypeItem chatTypeItem = new PolyvChatListAdapter.ChatTypeItem(customEvent, PolyvChatListAdapter.ChatTypeItem.TYPE_SEND, PolyvChatManager.SE_CUSTOMMESSAGE);
        addSendItemToList(chatTypeItem);
    }
    ...
    private PolyvCustomEvent.UserBean generateCustomEventUserBean() {
        PolyvCustomEvent.UserBean userBean = new PolyvCustomEvent.UserBean();
        userBean.setPic(chatManager.imageUrl);
        userBean.setNick(chatManager.nickName);
        userBean.setUserId(chatManager.userId);
        return userBean;
    }
    // </editor-fold> 
...
//        greetingText.acceptLoginEvent(loginEvent);
    ...
    private PolyvChatListAdapter.ChatTypeItem acceptCustomMessage(String event, String message, String socketListen, boolean isHistory) throws Exception {
        PolyvChatListAdapter.ChatTypeItem chatTypeItem = null;
        PolyvCustomEvent customEvent = null;
        int eventType = -1;
        switch (event) {
            case PolyvChatMessageEvent.MessageEvent.QMessage:
                Type qType = new TypeToken<PolyvCustomEvent<PolyvCustomQBean>>() {
                }.getType();
                customEvent = PolyvEventHelper.gson.fromJson(message, qType);
                if (customEvent.getUser() != null) {
                    if (chatManager.userId.equals(customEvent.getUser().getUserId()))
                        eventType = PolyvChatListAdapter.ChatTypeItem.TYPE_SEND;
                    else
                        eventType = PolyvChatListAdapter.ChatTypeItem.TYPE_RECEIVE;
                }
                break;
            case PolyvChatMessageEvent.MessageEvent.GiftMessage:
                Type giftType = new TypeToken<PolyvCustomEvent<PolyvCustomGiftBean>>() {
                }.getType();
                customEvent = PolyvEventHelper.gson.fromJson(message, giftType);
                eventType = PolyvChatListAdapter.ChatTypeItem.TYPE_TIPS;
                break;
            case PolyvChatMessageEvent.MessageEvent.RewardMessage:
                Type rewardType = new TypeToken<PolyvCustomEvent<PolyvCustomRewardBean>>() {
                }.getType();
                customEvent = PolyvEventHelper.gson.fromJson(message, rewardType);
                eventType = PolyvChatListAdapter.ChatTypeItem.TYPE_TIPS;
                break;
            default:
        ...
        if (customEvent != null
                && PolyvCustomEvent.VERSION_1 == customEvent.getVersion()
                && customEvent.getData() != null
                && customEvent.getUser() != null
                && (isHistory || !chatManager.userId.equals(customEvent.getUser().getUserId()))) {//非历史信息时，收到自己的信息时不添加
            chatTypeItem = new PolyvChatListAdapter.ChatTypeItem(customEvent, eventType, socketListen);
        }
        return chatTypeItem;
    }
        ...
        disposables.add(PolyvRxBus.get().toObservable(EventMessage.class).buffer(500, TimeUnit.MILLISECONDS).map(new Function<List<EventMessage>, List<PolyvChatListAdapter.ChatTypeItem>[]>() {
            ...
            public List<PolyvChatListAdapter.ChatTypeItem>[] apply(List<EventMessage> eventMessages) throws Exception {
                ...
                for (EventMessage eventMessage : eventMessages) {
                    ...
                    int eventType = -1;
                    if (PolyvChatManager.SE_CUSTOMMESSAGE.equals(socketListen)) {//自定义信息
                        try {
                            PolyvChatListAdapter.ChatTypeItem chatTypeItem = acceptCustomMessage(event, message, socketListen, false);
                            if (chatTypeItem != null) {
                                eventObject = chatTypeItem.object;
                                eventType = chatTypeItem.type;
                            }
                        } catch (Exception e) {
                        }
                    } else {//非自定义信息
...
//                                        }
                            ...
                            case PolyvChatManager.EVENT_LOGIN:
                                ...
                                if (loginEvent != null) {
                                    PolyvChatEventBus.get().post(loginEvent);
...
//                                    }
    ...
    public void showLikeNum(String num) {
        likeNum.setText(num);
    }
    // </editor-fold> 
}
